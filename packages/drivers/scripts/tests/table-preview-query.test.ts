...
import { buildTablePreviewClauses } from '../../src/database/shared/table-preview-query.ts';
...
assert.equal(
    sorted.whereSql,
    ` WHERE LOWER("status"::text) = $1 AND "amount" > $2 AND (LOWER("customer_name"::text) ILIKE $3 OR LOWER("note"::text) ILIKE $4)`,
);
