...
async function collectRoleGrantRelations(instance: ClickhouseDatasource) {
    ...
    const typeExpr = typeColumn ? `upper(${typeColumn})` : columns.has('user_name') ? `CASE WHEN ${user_name} != '' THEN 'USER' ELSE 'ROLE' END` : `'USER'`;
...
async function collectUserGlobalPrivileges(instance: ClickhouseDatasource, filterNames?: string[]) {
    ...
    for (const row of grantRows) {
        ...
        const isGlobal = (!database || database === '' || database === '*' || database === 'GLOBAL') && (!table || table === '' || table === '*');
...
async function collectUserScopedPrivileges(instance: ClickhouseDatasource, filterNames?: string[]) {
    ...
    for (const row of grantRows) {
        ...
        const isGlobal = (database === '*' || database.toUpperCase() === 'GLOBAL') && (table === '*' || table === '');
        if (isGlobal) continue;
...
function buildPrivilegeStatement(action: 'GRANT' | 'REVOKE', privilege: RolePrivilege, roleName: string, cluster?: string | null): string {
    const privilegeName = privilege.privilege || 'ALL';
    const columns = privilege.columns?.length ? `(${privilege.columns.map(column => quoteIdentifier(column)).join(', ')})` : '';
...
export async function listClickHouseUsers(instance: ClickhouseDatasource): Promise<ClickHouseUser[]> {
    ...
    const joinCondition = typeColumn ? `rg.${user_name} = u.name AND upper(rg.${typeColumn}) = 'USER'` : `rg.${user_name} = u.name`;
    ...
    return rows.map(row => {
        ...
        const globalPrivileges = name && globalPrivilegesMap.has(name) ? Array.from(globalPrivilegesMap.get(name)!).sort((a, b) => a.localeCompare(b, 'en')) : [];
...
export async function getClickHouseUser(instance: ClickhouseDatasource, name: string): Promise<ClickHouseUser | null> {
    ...
    const [globalPrivilegesMap, scopedPrivilegesMap] = await Promise.all([collectUserGlobalPrivileges(instance, [name]), collectUserScopedPrivileges(instance, [name])]);
...
export async function createClickHouseUser(instance: ClickhouseDatasource, payload: CreateUserPayload): Promise<void> {
    ...
    if (payload.roles?.length) {
        statements.push(applyClusterClause(`GRANT ${payload.roles.map(quoteIdentifier).join(', ')} TO ${nameIdentifier}`, cluster));
    }
    ...
    if (payload.defaultRoles) {
        if (payload.defaultRoles.length === 0) {
            ...
            statements.push(applyClusterClause(`ALTER USER ${nameIdentifier} DEFAULT ROLE ${payload.defaultRoles.map(quoteIdentifier).join(', ')}`, cluster));
        }
    }
...
export async function updateClickHouseUser(instance: ClickhouseDatasource, payload: UpdateUserPayload): Promise<void> {
    ...
    if (payload.roles) {
        ...
        if (toGrant.length) {
            statements.push(applyClusterClause(`GRANT ${toGrant.map(quoteIdentifier).join(', ')} TO ${identifier}`, cluster));
        }
        if (toRevoke.length) {
            statements.push(applyClusterClause(`REVOKE ${toRevoke.map(quoteIdentifier).join(', ')} FROM ${identifier}`, cluster));
        }
    }
    ...
    if (payload.defaultRoles) {
        if (payload.defaultRoles.length === 0) {
            ...
            statements.push(applyClusterClause(`ALTER USER ${identifier} DEFAULT ROLE ${payload.defaultRoles.map(quoteIdentifier).join(', ')}`, cluster));
        }
    }
    ...
    if (payload.newName && payload.newName !== payload.name) {
        const renameStatement = payload.cluster
            ? `RENAME USER ${identifier} ON CLUSTER ${quoteIdentifier(payload.cluster)} TO ${quoteIdentifier(payload.newName)}`
...
export async function grantUserGlobalPrivileges(instance: ClickhouseDatasource, payload: { name: string; privileges: string[] }): Promise<void> {
...
export async function revokeUserGlobalPrivileges(instance: ClickhouseDatasource, payload: { name: string; privileges: string[] }): Promise<void> {
    const privileges = normalizePrivilegeNames(payload.privileges);
    if (!privileges.length) return;
    const scoped = privileges.filter(priv => PRIVILEGES_REQUIRING_SCOPE.has(priv));
    const global = privileges.filter(priv => !PRIVILEGES_REQUIRING_SCOPE.has(priv));
...
export async function grantRoleGlobalPrivileges(instance: ClickhouseDatasource, payload: { name: string; privileges: string[] }): Promise<void> {
    const privileges = normalizePrivilegeNames(payload.privileges);
    if (!privileges.length) return;
    const scoped = privileges.filter(priv => PRIVILEGES_REQUIRING_SCOPE.has(priv));
    const global = privileges.filter(priv => !PRIVILEGES_REQUIRING_SCOPE.has(priv));
...
export async function revokeRoleGlobalPrivileges(instance: ClickhouseDatasource, payload: { name: string; privileges: string[] }): Promise<void> {
    const privileges = normalizePrivilegeNames(payload.privileges);
    if (!privileges.length) return;
    const scoped = privileges.filter(priv => PRIVILEGES_REQUIRING_SCOPE.has(priv));
    const global = privileges.filter(priv => !PRIVILEGES_REQUIRING_SCOPE.has(priv));
...
export async function updateClickHouseRole(instance: ClickhouseDatasource, payload: UpdateRolePayload): Promise<void> {
    ...
    if (payload.newName && payload.newName !== payload.name) {
        const renameStatement = cluster
            ? `RENAME ROLE ${quoteIdentifier(payload.name)} ON CLUSTER ${quoteIdentifier(cluster)} TO ${quoteIdentifier(payload.newName)}`
...
export type ClickhousePrivilegesImpl = {
    ...
    revokeUserScopedPrivileges: (payload: { name: string; privileges: string[]; database: string; object?: string | null; scopeType: ScopedPrivilegeScope }) => Promise<void>;
    ...
    revokeRoleScopedPrivileges: (payload: { name: string; privileges: string[]; database: string; object?: string | null; scopeType: ScopedPrivilegeScope }) => Promise<void>;
};
...
export function getClickhousePrivilegesImpl(instance: ClickhouseDatasource): ClickhousePrivilegesImpl {
    return {
        ...
        grantUserGlobalPrivileges: (payload: { name: string; privileges: string[] }) => grantUserGlobalPrivileges(instance, payload),
        revokeUserGlobalPrivileges: (payload: { name: string; privileges: string[] }) => revokeUserGlobalPrivileges(instance, payload),
        grantUserScopedPrivileges: (payload: {
            name: string;
            privileges: string[];
            database: string;
        ...
        revokeUserScopedPrivileges: (payload: { name: string; privileges: string[]; database: string; object?: string | null; scopeType: ScopedPrivilegeScope }) =>
        ...
        grantRoleGlobalPrivileges: (payload: { name: string; privileges: string[] }) => grantRoleGlobalPrivileges(instance, payload),
        revokeRoleGlobalPrivileges: (payload: { name: string; privileges: string[] }) => revokeRoleGlobalPrivileges(instance, payload),
        grantRoleScopedPrivileges: (payload: {
            name: string;
            privileges: string[];
            database: string;
        ...
        revokeRoleScopedPrivileges: (payload: { name: string; privileges: string[]; database: string; object?: string | null; scopeType: ScopedPrivilegeScope }) =>
