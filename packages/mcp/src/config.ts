...
export function emptyConfig(): DoryMcpConfig {
    return {
        version: 1,
        credentials: {},
    };
...
export async function readConfig(configPath = getConfigPath()): Promise<DoryMcpConfig> {
    try {
...
export async function writeConfig(config: DoryMcpConfig, configPath = getConfigPath()) {
...
export async function saveCredential(origin: string, credential: DoryMcpCredential, configPath = getConfigPath()) {
    const config = await readConfig(configPath);
    config.credentials[origin] = credential;
    await writeConfig(config, configPath);
}

export async function removeCredential(origin: string, configPath = getConfigPath()) {
    const config = await readConfig(configPath);
    delete config.credentials[origin];
    await writeConfig(config, configPath);
}

export async function deleteConfig(configPath = getConfigPath()) {
    await rm(configPath, { force: true });
}

export async function resolveCredential(url?: string | null, env: NodeJS.ProcessEnv = process.env, configPath = getConfigPath(env)): Promise<ResolvedCredential | null> {
