...
export function createMcpProxyManager({ log, logWarn, logError }: CreateMcpProxyManagerOptions) {
    ...
    function migrateLegacyEnabledForUser(userId: string) {
        if (mcpProxyStore.get('enabled') !== true) {
            return false;
        }

        log('[electron] migrating legacy MCP proxy enabled state for user:', userId);
    ...
    function isEnabledForUser(userId?: string | null) {
        ...
        return enabledByUserId[userId] === true || migrateLegacyEnabledForUser(userId);
    }
