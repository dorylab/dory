...
const databaseSummarySchema = z.object({
    ...
    engine: z.enum(['clickhouse', 'doris', 'duckdb', 'mariadb', 'mysql', 'postgres', 'sqlite', 'unknown']),
    ...
    return withUserAndOrganizationHandler(async ({ userId, organizationId }) => {
        ...
        try {
            ...
            const engine = isPostgresFamilyConnectionType(config.type)
                ? 'postgres'
                : ((config.type ?? 'unknown') as 'clickhouse' | 'doris' | 'duckdb' | 'mariadb' | 'mysql' | 'postgres' | 'sqlite' | 'unknown');
