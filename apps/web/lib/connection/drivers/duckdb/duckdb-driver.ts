...
export async function executeDuckDbQuery<Row = any>(handle: DuckDbConnectionHandle, sql: string, params?: DriverQueryParams): Promise<QueryResult<Row>> {
    ...
    const limitedSql = isReadStatement(compiledSql) ? enforceSelectLimit(compiledSql, MAX_RESULT_ROWS) : compiledSql;
    const started = Date.now();
    ...
    return {
        ...
        limited: isReadStatement(compiledSql) && rows.length >= MAX_RESULT_ROWS,
        limit: isReadStatement(compiledSql) ? MAX_RESULT_ROWS : undefined,
        tookMs: Date.now() - started,
    };
}
