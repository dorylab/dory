function projectConnectionForTools(item: any) {
    return {
...
export function projectConnectionListForTools(output: { connections?: any[] }) {
    return {
        connections: (output.connections ?? []).map(projectConnectionForTools),
    };
}

export function projectConnectionGetForTools(output: any) {
    return {
        connection: projectConnectionForTools(output),
    };
}
